...
public abstract class FlowImpl 
	...
	/**
	 * Sets the map of named objects that will be used to execute flow steps.
	 * 
     ...
     * @param viewClass the class which implements the view.
     */
	protected final void addViewConfiguration(
			String viewName
			) {
	...
	/**
	 * Adds the basic details of a controller configuration. Addition of resolution mappings
	 ...
	 */
	protected final void addControllerConfiguration(
	...
	/**
	 * Adds a controller configuration.
	 * @param controllerName the name of the controller, which must be unique.
	 * @param managedObjectName the name of the managed object which provides the controller function.
	 */
	protected final void addControllerConfiguration(
			String controllerName, 
			String managedObjectName) {
		addControllerConfiguration(controllerName, managedObjectName, null);
	}
	
	/**
	 * A convenience version of {@link #addResolutionMapping(String, String, Guard, Destination)}
	 * where the destination is the named controller.
	 * 
	...
	private FlowOutcome handleResolution(
		...
		log.debug("Processing controller resolution: flow step="
		 ...
		 */
			...
			try {
				...
				throw new Error("Error trying to instantiate the next flow resolution factory: " 
						+ subFlowDestination.destinationFlowFactoryClass.getName(), e);
			} 
		}
		/*
	...
	private Destination getDestination(
		...
		if (mappings == null) {
			throw new IllegalArgumentException("Neither a specific controller mapping nor a global mapping could be found for resolution name: " 
					+ resolutionName + "; Flow resolution factory class name: " + this.getClass().getName());
		}
